...
Write-Host " Paper Trading 404 Fix v2"
Write-Host " Duplicate-ID Guard + Session Schema Fix"
Write-Host "============================================================"
...
$generatorPath = Join-Path $automationDir "gpt_quant_v91_paper_trading_404_fix_v2.py"
$sqlPath = Join-Path $supabaseDir "GPT_QUANT_V91_PAPER_TRADING_404_FIX_V2.sql"
$validatorPath = Join-Path $automationDir "gpt_quant_v91_paper_trading_schema_check_v2.py"
$workflowPath = Join-Path $workflowDir "gpt-quant-v91-paper-trading-404-fix-v2.yml"

if (-not (Test-Path $targetScript)) {
    throw "Missing original file: $targetScript"
}

$backupPath = "$targetScript.pre404fixv2.bak"
...
TABLE = "gpt_quant_v91_paper_sessions"
...
$workflow = @'
name: GPT Quant V9.1 Paper Trading 404 Fix v2 - Schema Check
...
jobs:
  paper-trading-schema-check-v2:
    ...
    steps:
      ...
      - name: Validate Paper Trading session schema endpoint
        run: python automation/gpt_quant_v91_paper_trading_schema_check_v2.py
'@
...
Write-Host ""
Write-Host "Generating Paper Trading v2 SQL from current upsert payload..."
python $generatorPath

if ($LASTEXITCODE -ne 0) {
    throw "Failed to inspect current Paper Trading payload."
}
...
Write-Host "2. Open:"
Write-Host "   supabase\GPT_QUANT_V91_PAPER_TRADING_404_FIX_V2.sql"
Write-Host "3. Copy ALL -> Paste -> Run"
Write-Host ""
Write-Host "Expected:"
Write-Host "   table_exists = true"
Write-Host "   conflict_key = session_date"
Write-Host "   payload_contains_id = true/false"
Write-Host ""
Write-Host "Then:"
Write-Host "   GitHub Desktop -> Commit -> Push origin"
Write-Host "   Actions -> GPT Quant V9.1 Paper Trading 404 Fix v2 - Schema Check"
Write-Host ""
Write-Host "If green:"
Write-Host "   Rerun GPT Quant V9.1 Optimization Suite"
Write-Host ""
Write-Host "Important:"
Write-Host "   v2 prevents duplicate id column creation."
Write-Host "   UNIQUE(session_date) is created."
Write-Host "   No synthetic paper session rows are inserted."
