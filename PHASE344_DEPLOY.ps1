...
Write-Host "============================================================"
Write-Host " GPT Quant V9.2 Phase 3.4.4 Deployment"
Write-Host " Human Approval Gate + Release Authorization"
Write-Host "============================================================"
...
$pyPath = Join-Path $automationDir "paper_trading_phase344_human_approval_gate.py"
$ymlPath = Join-Path $workflowDir "gpt-quant-v92-paper-trading-phase344.yml"
...
$workflow = @'
name: GPT Quant V9.2 Paper Trading Phase 3.4.4 - Human Approval Gate + Release Authorization
...
on:
  workflow_dispatch:
    inputs:
      ...
      action:
        description: "Manual release-control action"
        required: true
        default: "evaluate"
        type: choice
        options:
      ...
      approver:
        description: "Human approver identifier; required for approve/revoke"
        required: false
        default: ""
        type: string

      confirmation_text:
        description: "Approve: APPROVE PRODUCTION PAPER | Revoke: REVOKE PRODUCTION PAPER"
        required: false
        default: ""
        type: string
...
concurrency:
  group: gpt-quant-v92-phase344-human-approval-gate
...
jobs:
  human-approval-gate:
    ...
    env:
      ...
      PHASE344_ACTION: ${{ inputs.action || 'evaluate' }}
      PHASE344_APPROVER: ${{ inputs.approver || '' }}
      PHASE344_CONFIRMATION_TEXT: ${{ inputs.confirmation_text || '' }}
    ...
    steps:
      ...
      - name: Validate Phase 3.4.4 environment
        shell: bash
        run: |
          set -euo pipefail
      ...
      - name: Validate manual approval input
        if: ${{ inputs.action == 'approve_production_paper' }}
        shell: bash
        run: |
          set -euo pipefail
          test -n "${PHASE344_APPROVER:-}" || {
            echo "::error::Approver is required."
            exit 1
          }
          test "${PHASE344_CONFIRMATION_TEXT}" = "APPROVE PRODUCTION PAPER" || {
            echo "::error::Approval confirmation phrase does not match."
            exit 1
          }

      - name: Validate manual revocation input
        if: ${{ inputs.action == 'revoke_production_paper' }}
        shell: bash
        run: |
          set -euo pipefail
          test -n "${PHASE344_APPROVER:-}" || {
          ...
          test "${PHASE344_CONFIRMATION_TEXT}" = "REVOKE PRODUCTION PAPER" || {
            echo "::error::Revocation confirmation phrase does not match."
            exit 1
          }

      - name: Run Phase 3.4.4 Human Approval Gate
        run: python automation/v92/paper_trading_phase344_human_approval_gate.py

      - name: Upload Phase 3.4.4 audit artifacts
        if: always()
        uses: actions/upload-artifact@v4
        with:
          name: phase344-human-approval-gate-${{ github.run_id }}
          path: |
            phase34_result.json
            phase34_summary.md
...
Write-Host ""
Write-Host "============================================================"
Write-Host " PHASE 3.4.4 DEPLOYMENT READY"
Write-Host "============================================================"
Write-Host ""
Write-Host "Gate behavior:"
Write-Host "  evaluate:"
Write-Host "    <5/5 -> NOT READY / LOCKED"
Write-Host "    5/5  -> READY FOR HUMAN APPROVAL / LOCKED"
Write-Host ""
Write-Host "  approve_production_paper:"
Write-Host "    Requires 5/5 + READY_FOR_HUMAN_APPROVAL"
Write-Host "    Requires named approver"
Write-Host "    Requires exact phrase: APPROVE PRODUCTION PAPER"
Write-Host "    Authorizes PRODUCTION PAPER only"
Write-Host ""
Write-Host "  revoke_production_paper:"
Write-Host "    Requires named approver"
Write-Host "    Requires exact phrase: REVOKE PRODUCTION PAPER"
Write-Host ""
Write-Host "Safety:"
Write-Host "  Automatic approval: DISABLED"
Write-Host "  Broker trading: DISABLED"
Write-Host "  Real-money trading: DISABLED"
Write-Host "  No scheduled approval workflow"
Write-Host ""
Write-Host "Next:"
Write-Host "  GitHub Desktop -> Commit -> Push origin"
Write-Host "  GitHub Actions -> Phase 3.4.4"
Write-Host "  FIRST TEST: action=evaluate only"
