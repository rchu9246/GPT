...
Write-Host "PHASE375 V2 - Runtime Supervision Canonical State Reconstruction Fix" -ForegroundColor Cyan
Write-Host "Safety boundary: PAPER ONLY / broker submission DISABLED / real money DISABLED" -ForegroundColor Green

$repo = (Get-Location).Path
$pyRel = "automation/v92/paper_trading_phase375_production_paper_multi_cycle_stability_evidence_qualification.py"
$ymlRel = ".github/workflows/gpt-quant-v92-paper-trading-phase375-production-paper-multi-cycle-stability-evidence-qualification.yml"
$pyPath = Join-Path $repo $pyRel
$ymlPath = Join-Path $repo $ymlRel

if (-not (Test-Path $pyPath)) { throw "Missing target: $pyPath" }
if (-not (Test-Path $ymlPath)) { throw "Missing target: $ymlPath" }

$stamp = Get-Date -Format "yyyyMMdd-HHmmss"
$backup = Join-Path $repo ".phase375-v2-runtime-reconstruction-backup-$stamp"
...
$required = @(
  'runtime_supervision_reconstruct',
  'CANONICAL_ROW_PRESENT_NON_BLOCKING',
  'EXPLICIT_BLOCK_STATE',
  'EXPLICIT_READY_STATE',
  'BROKER_ORDER_SUBMISSION_ENABLED = False',
  'REAL_MONEY_TRADING_ENABLED = False',
  'HISTORICAL_REWRITE_ALLOWED = False',
...
Write-Host "Runtime canonical state reconstruction: PASS" -ForegroundColor Green
Write-Host "Fail-closed explicit blocker handling: PASS" -ForegroundColor Green
Write-Host "Observation-state preservation: PASS" -ForegroundColor Green
...
Write-Host ""
Write-Host "PHASE375 V2 RUNTIME SUPERVISION CANONICAL STATE RECONSTRUCTION FIX DEPLOYMENT COMPLETE" -ForegroundColor Cyan
Write-Host "No Supabase SQL is required." -ForegroundColor Green
Write-Host "Backup: $backup"
Write-Host ""
Write-Host "Expected next healthy state:"
Write-Host "  MULTI_CYCLE_STABILITY_OBSERVING"
Write-Host "  runtime_supervision: PASS"
Write-Host "  Qualified: NO until evidence thresholds are genuinely met"
Write-Host ""
Write-Host "NEXT:"
Write-Host '1. git add "automation/v92/paper_trading_phase375_production_paper_multi_cycle_stability_evidence_qualification.py"'
Write-Host '2. git add ".github/workflows/gpt-quant-v92-paper-trading-phase375-production-paper-multi-cycle-stability-evidence-qualification.yml"'
Write-Host '3. git status'
Write-Host '4. git commit -m "Fix Phase 375 V2 runtime supervision canonical state reconstruction"'
Write-Host '5. git push origin main'
Write-Host '6. Run a NEW Phase 3.7.5 workflow on main (do not re-run an old commit).'
