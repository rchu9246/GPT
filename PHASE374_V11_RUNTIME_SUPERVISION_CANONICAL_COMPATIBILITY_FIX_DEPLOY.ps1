...
$backup = Join-Path $root ".phase374-v11-backup-$stamp"
...
$pyText = Get-Content -LiteralPath $pyPath -Raw
if ($pyText -notmatch "runtime_supervision_ready" -or $pyText -notmatch "CONTINUE_WITH_OBSERVATION" -or $pyText -notmatch "portfolio_scoped") {
    throw "Runtime supervision canonical compatibility contract missing."
}
Write-Host "Runtime supervision canonical compatibility: PASS" -ForegroundColor Green
...
Write-Host ""
Write-Host "PHASE374 V1.1 RUNTIME SUPERVISION COMPATIBILITY FIX DEPLOYMENT COMPLETE" -ForegroundColor Cyan
