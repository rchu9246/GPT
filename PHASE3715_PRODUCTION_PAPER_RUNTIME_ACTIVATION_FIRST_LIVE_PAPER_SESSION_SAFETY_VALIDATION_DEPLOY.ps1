...
Write-Host "PHASE3715 - Production Paper Runtime Activation + First Live Paper Session Safety Validation" -ForegroundColor Cyan
Write-Host "Safety boundary: PAPER ONLY / broker submission DISABLED / real money DISABLED" -ForegroundColor Green
Write-Host "No runtime activation before genuine Phase 3.7.14 gate readiness" -ForegroundColor Green

$repo = (Get-Location).Path
$pyRel = "automation/v92/paper_trading_phase3715_production_paper_runtime_activation_first_live_paper_session_safety_validation.py"
$ymlRel = ".github/workflows/gpt-quant-v92-paper-trading-phase3715-production-paper-runtime-activation-first-live-paper-session-safety-validation.yml"

$pyPath = Join-Path $repo $pyRel
$ymlPath = Join-Path $repo $ymlRel

$stamp = Get-Date -Format "yyyyMMdd-HHmmss"
$backup = Join-Path $repo ".phase3715-backup-$stamp"
...
$ymlText = @'
name: GPT Quant Phase 3.7.15 - Production Paper Runtime Activation First Live Paper Session Safety Validation
...
concurrency:
  group: phase3715-first-live-paper-session-safety-validation
  cancel-in-progress: false

jobs:
  first-live-paper-session-safety-validation:
    ...
    env:
      ...
      WF3714: gpt-quant-v92-paper-trading-phase3714-production-paper-qualification-3of3-promotion-finalization-paper-runtime-activation-gate.yml
    ...
    steps:
      ...
      - name: Compile Phase 3.7.15
        run: python -m py_compile automation/v92/paper_trading_phase3715_production_paper_runtime_activation_first_live_paper_session_safety_validation.py

      - name: Validate Phase 3.7.15 safety contract
        shell: bash
        run: |
          set -euo pipefail
          f="automation/v92/paper_trading_phase3715_production_paper_runtime_activation_first_live_paper_session_safety_validation.py"
          grep -q 'SYNTHETIC_QUALIFICATION_ALLOWED = False' "$f"
          grep -q 'MANUAL_COUNTER_INCREMENT_ALLOWED = False' "$f"
          grep -q 'RUNTIME_ACTIVATION_WITHOUT_GATE_READY_ALLOWED = False' "$f"
          grep -q 'FIRST_LIVE_PAPER_SESSION_WITHOUT_SAFETY_PASS_ALLOWED = False' "$f"
          grep -q 'PAPER_RUNTIME_ACTIVATION_ARMED_WAITING' "$f"
          grep -q 'PAPER_RUNTIME_ACTIVATION_VALIDATED' "$f"

      - name: Confirm Phase 3.7.14 successful lineage
        shell: bash
        env:
        ...
        run: |
          ...
          result="$(gh api -H "Accept: application/vnd.github+json" \
            "/repos/${GITHUB_REPOSITORY}/actions/workflows/${WF3714}/runs?branch=main&per_page=20" \
            --jq '[.workflow_runs[] | select(.conclusion == "success")][0].conclusion // empty')"
          [ "$result" = "success" ] || { echo "::error::No successful Phase 3.7.14 run found."; exit 1; }

      - name: Evaluate runtime activation and first live paper session safety
        id: phase3715
        continue-on-error: true
        run: python automation/v92/paper_trading_phase3715_production_paper_runtime_activation_first_live_paper_session_safety_validation.py

      - name: Publish summary
        if: always()
        shell: bash
        run: |
          if [ -f artifacts/phase3715/phase3715_summary.md ]; then
            cat artifacts/phase3715/phase3715_summary.md >> "$GITHUB_STEP_SUMMARY"
          fi

      - name: Upload Phase 3.7.15 evidence
        if: always()
        uses: actions/upload-artifact@v4
        with:
          name: phase3715-first-live-paper-session-safety-validation
          path: artifacts/phase3715
          if-no-files-found: warn
          retention-days: 90

      - name: Enforce Phase 3.7.15 result
        if: always()
        shell: bash
        run: |
          if [ "${{ steps.phase3715.outcome }}" != "success" ]; then
            echo "Phase 3.7.15 first live paper session safety validation is BLOCKED."
            exit 1
          fi
          echo "Phase 3.7.15 first live paper session safety validation is healthy."
'@
...
$required = @(
  'PAPER_RUNTIME_ACTIVATION_ARMED_WAITING',
  'PAPER_RUNTIME_ACTIVATION_VALIDATED',
  'FIRST_LIVE_PAPER_SESSION_SAFETY_BLOCKED',
  'SYNTHETIC_QUALIFICATION_ALLOWED = False',
  'MANUAL_COUNTER_INCREMENT_ALLOWED = False',
  'RUNTIME_ACTIVATION_WITHOUT_GATE_READY_ALLOWED = False',
  'FIRST_LIVE_PAPER_SESSION_WITHOUT_SAFETY_PASS_ALLOWED = False',
  'gpt-quant-v92-paper-trading-phase3714-production-paper-qualification-3of3-promotion-finalization-paper-runtime-activation-gate.yml'
)
...
Write-Host "Phase 3.7.14 lineage bridge: PASS" -ForegroundColor Green
Write-Host "Canonical 3/3 runtime activation contract: PASS" -ForegroundColor Green
Write-Host "First live paper session safety contract: PASS" -ForegroundColor Green
...
Write-Host "Runtime activation pre-gate prohibition: PASS" -ForegroundColor Green
Write-Host "Broker/real-money safety lock: PASS" -ForegroundColor Green
Write-Host ""
Write-Host "PHASE3715 DEPLOYMENT COMPLETE" -ForegroundColor Cyan
Write-Host "No Supabase SQL schema change is required." -ForegroundColor Green
Write-Host ""
Write-Host "Expected current state:"
Write-Host "  PAPER_RUNTIME_ACTIVATION_ARMED_WAITING"
Write-Host "  Runtime Active: NO"
Write-Host "  First Live Paper Session Ready: NO"
Write-Host "  Reason: waiting for genuine canonical 3/3 + promotion readiness"
Write-Host ""
Write-Host "Only after genuine gate readiness:"
Write-Host "  PAPER_RUNTIME_ACTIVATION_VALIDATED"
Write-Host "  Runtime Active: YES"
Write-Host "  First Live Paper Session Ready: YES"
Write-Host ""
Write-Host "Even after activation:"
...
Write-Host "  Weekdays 17:50 UTC / 01:50 Asia-Taipei"
Write-Host ""
Write-Host "Backup: $backup"
Write-Host ""
Write-Host "NEXT:"
Write-Host '1. git add "automation/v92/paper_trading_phase3715_production_paper_runtime_activation_first_live_paper_session_safety_validation.py"'
Write-Host '2. git add ".github/workflows/gpt-quant-v92-paper-trading-phase3715-production-paper-runtime-activation-first-live-paper-session-safety-validation.yml"'
Write-Host '3. git diff --cached --name-only'
Write-Host '4. git commit -m "Deploy Phase 3715 production paper runtime activation first live paper session safety validation"'
Write-Host '5. git push origin main'
Write-Host '6. Run a NEW Phase 3.7.15 workflow on main.'
