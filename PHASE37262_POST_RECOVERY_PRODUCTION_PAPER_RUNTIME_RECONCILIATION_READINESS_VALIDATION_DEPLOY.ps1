...
Purpose
-------
Validate that the successful Phase 3.7.2.6.1 canonical reconstruction is
actually consumable by the Production Paper runtime before re-running the
higher-level Phase 3.7.2.6 requalification.

This phase is READ-ONLY.

It validates:
- Activation canonical state is readable and ACTIVE.
- Master-cycle canonical state is readable.
- Runtime supervision is not currently REVOKED / FAIL_CLOSED.
- Latest reconstructed state is coherent.
- Production Paper continuation readiness can be classified.
- Broker order submission remains disabled.
- Real-money trading remains disabled.
- Historical rewrite remains prohibited.
...
function Fail([string]$Text) {
    Write-Host ""
    Write-Host "PHASE37262_FATAL: $Text" -ForegroundColor Red
    exit 1
}
...
Section "GPT Quant V9.2 — Phase 3.7.2.6.2 Runtime Reconciliation + Readiness Validation"
...
$automationTarget = Join-Path $repo "automation\v92\paper_trading_phase37262_post_recovery_production_paper_runtime_reconciliation_readiness_validation.py"
$workflowTarget = Join-Path $repo ".github\workflows\gpt-quant-v92-paper-trading-phase37262-post-recovery-production-paper-runtime-reconciliation-readiness-validation.yml"

$stamp = Get-Date -Format "yyyyMMdd-HHmmss"
$backupDir = Join-Path $repo ".phase37262-backup-$stamp"
New-Item -ItemType Directory -Force -Path $backupDir | Out-Null

foreach ($target in @($automationTarget, $workflowTarget)) {
    if (Test-Path $target) {
        Copy-Item $target (Join-Path $backupDir ([IO.Path]::GetFileName($target))) -Force
    }
}

Section "1/4 — Write Phase 3.7.2.6.2 runtime validator"
...
WriteUtf8 $automationTarget $py
Write-Host "Wrote: $automationTarget" -ForegroundColor Green

Section "2/4 — Write GitHub Actions workflow"

$yml = @'
name: GPT Quant Phase 3.7.2.6.2 - Post Recovery Production Paper Runtime Reconciliation Readiness Validation
...
concurrency:
  group: phase37262-post-recovery-runtime-readiness
  cancel-in-progress: false

jobs:
  post-recovery-runtime-readiness:
    ...
    steps:
      ...
      - name: Compile Phase 3.7.2.6.2
        run: |
          python -m py_compile automation/v92/paper_trading_phase37262_post_recovery_production_paper_runtime_reconciliation_readiness_validation.py

      - name: Validate Phase 3.7.2.6.2 safety contract
        shell: bash
        run: |
          set -euo pipefail
          test -n "${SUPABASE_URL:-}"
          ...
          FILE="automation/v92/paper_trading_phase37262_post_recovery_production_paper_runtime_reconciliation_readiness_validation.py"

          grep -q 'PHASE37262_POST_RECOVERY_PRODUCTION_PAPER_RUNTIME_RECONCILIATION_READINESS_VALIDATION' "$FILE"
          grep -q 'phase37261_reconstruction_audit_v92' "$FILE"
          ...
          echo "Phase 3.7.2.6.2 safety contract: PASS"

      - name: Execute Phase 3.7.2.6.2
        shell: bash
        run: |
          mkdir -p artifacts/phase37262

          python automation/v92/paper_trading_phase37262_post_recovery_production_paper_runtime_reconciliation_readiness_validation.py \
            --strategy-version "$STRATEGY_VERSION" \
            --portfolio-id "$PORTFOLIO_ID" \
            | tee artifacts/phase37262/console.md

      - name: Publish summary
        if: always()
        shell: bash
        run: |
          if [ -f artifacts/phase37262/summary.md ]; then
            cat artifacts/phase37262/summary.md >> "$GITHUB_STEP_SUMMARY"
          elif [ -f artifacts/phase37262/console.md ]; then
            cat artifacts/phase37262/console.md >> "$GITHUB_STEP_SUMMARY"
          fi
      ...
      - name: Upload evidence
        ...
        with:
          name: phase37262-runtime-reconciliation-readiness
          path: artifacts/phase37262/
          if-no-files-found: warn
          retention-days: 120
'@

WriteUtf8 $workflowTarget $yml
Write-Host "Wrote: $workflowTarget" -ForegroundColor Green

Section "3/4 — Static validation"

if (Get-Command python -ErrorAction SilentlyContinue) {
    & python -m py_compile $automationTarget
} elseif (Get-Command py -ErrorAction SilentlyContinue) {
    & py -3 -m py_compile $automationTarget
} else {
    Fail "Python not found in PATH."
}

if ($LASTEXITCODE -ne 0) {
    Fail "Phase 3.7.2.6.2 Python compile failed."
}

$combined = (Get-Content -LiteralPath $automationTarget -Raw) + "`n" + (Get-Content -LiteralPath $workflowTarget -Raw)

foreach ($token in @(
    "PHASE37262_POST_RECOVERY_PRODUCTION_PAPER_RUNTIME_RECONCILIATION_READINESS_VALIDATION",
    ...
    if (-not $combined.Contains($token)) {
        Fail "Required Phase 3.7.2.6.2 token missing: $token"
    }
}

Write-Host "Python compile: PASS" -ForegroundColor Green
Write-Host "Activation canonical validation contract: PASS" -ForegroundColor Green
Write-Host "Master-cycle canonical validation contract: PASS" -ForegroundColor Green
Write-Host "Runtime supervision validation contract: PASS" -ForegroundColor Green
Write-Host "V6.3 reconstruction relation validation contract: PASS" -ForegroundColor Green
Write-Host "Paper-only safety boundary: PASS" -ForegroundColor Green

Section "4/4 — Deployment complete"

Write-Host "PHASE37262 DEPLOYMENT COMPLETE" -ForegroundColor Cyan
Write-Host ""
Write-Host "Generated:" -ForegroundColor Green
Write-Host "  automation/v92/paper_trading_phase37262_post_recovery_production_paper_runtime_reconciliation_readiness_validation.py"
Write-Host "  .github/workflows/gpt-quant-v92-paper-trading-phase37262-post-recovery-production-paper-runtime-reconciliation-readiness-validation.yml"
Write-Host ""
Write-Host "No Supabase SQL is required." -ForegroundColor Green
Write-Host ""
Write-Host "Next:" -ForegroundColor Yellow
Write-Host "  1) Commit + Push ONLY the two generated runtime files."
Write-Host "  2) Run GitHub Action:"
Write-Host "     GPT Quant Phase 3.7.2.6.2 - Post Recovery Production Paper Runtime Reconciliation Readiness Validation"
Write-Host "  3) Target:"
Write-Host "     Production Paper Readiness: READY"
Write-Host "     Activation Validation: PASS"
Write-Host "     Master Cycle Validation: PASS"
Write-Host "     Runtime Supervision Validation: PASS"
Write-Host "     Reconstruction Validation: PASS"
Write-Host "  4) Only after PASS, re-run Phase 3.7.2.6."
Write-Host ""
...
if ($AutoGit) {
    ...
    & git add -- $automationTarget $workflowTarget
    & git commit -m "Deploy Phase 37262 post-recovery runtime readiness validation"
    if ($LASTEXITCODE -ne 0) {
