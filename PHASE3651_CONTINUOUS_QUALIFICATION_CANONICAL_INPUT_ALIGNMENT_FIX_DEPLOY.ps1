...
<#
PHASE3651_CONTINUOUS_QUALIFICATION_CANONICAL_INPUT_ALIGNMENT_FIX_DEPLOY.ps1

GPT Quant V9.2
Phase 3.6.5.1 - Production Paper Autonomous Promotion Control + Continuous Qualification Engine
...
Creates:
  ...
  supabase/PHASE3651_CONTINUOUS_QUALIFICATION_CANONICAL_INPUT_ALIGNMENT_FIX.sql
  .github/workflows/gpt-quant-v92-paper-trading-phase3651-continuous-qualification-canonical-input-alignment-fix.yml
...
Write-Section "GPT Quant V9.2 - Phase 3.6.5.1"
Write-Host "Production Paper Autonomous Promotion Control + Continuous Qualification Engine"
Write-Host "Repository: $Repo"

$PyPath  = Join-Path $Repo "automation\v92\paper_trading_phase3651_continuous_qualification_canonical_input_alignment_fix.py"
$SqlPath = Join-Path $Repo "supabase\PHASE3651_CONTINUOUS_QUALIFICATION_CANONICAL_INPUT_ALIGNMENT_FIX.sql"
$YmlPath = Join-Path $Repo ".github\workflows\gpt-quant-v92-paper-trading-phase3651-continuous-qualification-canonical-input-alignment-fix.yml"
...
CONTRACT = "PHASE3651_CONTINUOUS_QUALIFICATION_CANONICAL_INPUT_ALIGNMENT_FIX"
...
def main() -> int:
    ...
    print(f"# GPT Quant V9.2 Paper Trading - Phase 3.6.5.1")
    print()
...
$Yaml = @'
name: GPT Quant Phase 3.6.5.1 - Production Paper Autonomous Promotion Control Continuous Qualification Engine
...
jobs:
  autonomous-promotion-control-continuous-qualification:
    ...
    steps:
      ...
      - name: Compile Phase 3.6.5.1
        shell: bash
        run: |
          python -m py_compile automation/v92/paper_trading_phase3651_continuous_qualification_canonical_input_alignment_fix.py

      - name: Run Phase 3.6.5.1
        shell: bash
        run: |
          mkdir -p artifacts/phase365
          python automation/v92/paper_trading_phase3651_continuous_qualification_canonical_input_alignment_fix.py \
      ...
      - name: Upload Phase 3.6.5.1 evidence
        if: always()
...
$required = @(
    "PHASE3651_CONTINUOUS_QUALIFICATION_CANONICAL_INPUT_ALIGNMENT_FIX",
    "paper_continuous_qualification_v92",
...
foreach ($token in $required) {
    if ($combined -notmatch [regex]::Escape($token)) {
        throw "Phase 3.6.5.1 contract scan failed. Missing token: $token"
    }
}
...
Write-Host "Phase 3.6.5.1 continuous-qualification contract scan: PASS" -ForegroundColor Green
...
Write-Host "  supabase/PHASE3651_CONTINUOUS_QUALIFICATION_CANONICAL_INPUT_ALIGNMENT_FIX.sql"
Write-Host "  .github/workflows/gpt-quant-v92-paper-trading-phase3651-continuous-qualification-canonical-input-alignment-fix.yml"
Write-Host ""
Write-Host "Next:"
Write-Host "  1. Run the generated SQL once in Supabase SQL Editor."
Write-Host "  2. Commit and push generated files to main."
Write-Host "  3. Run the Phase 3.6.5.1 GitHub Action."
Write-Host ""
