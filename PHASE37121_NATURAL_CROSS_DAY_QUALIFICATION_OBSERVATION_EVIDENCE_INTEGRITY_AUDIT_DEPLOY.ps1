...
Write-Host "PHASE37121 - Natural Cross-Day Qualification Observation + Evidence Integrity Audit" -ForegroundColor Cyan
Write-Host "Mode: READ-ONLY observation / no synthetic evidence / no counter mutation" -ForegroundColor Green
Write-Host "Safety boundary: PAPER ONLY / broker submission DISABLED / real money DISABLED" -ForegroundColor Green

$repo = (Get-Location).Path
$pyRel = "automation/v92/paper_trading_phase37121_natural_cross_day_qualification_observation_evidence_integrity_audit.py"
$ymlRel = ".github/workflows/gpt-quant-v92-paper-trading-phase37121-natural-cross-day-qualification-observation-evidence-integrity-audit.yml"

$pyPath = Join-Path $repo $pyRel
$ymlPath = Join-Path $repo $ymlRel

$stamp = Get-Date -Format "yyyyMMdd-HHmmss"
$backup = Join-Path $repo ".phase37121-backup-$stamp"
...
'@
$ymlText = @'
name: GPT Quant Phase 3.7.12.1 - Natural Cross-Day Qualification Observation Evidence Integrity Audit

on:
  workflow_dispatch:
  schedule:
    # Weekdays 16:50 UTC / 00:50 Asia-Taipei.
    - cron: "50 16 * * 1-5"
...
concurrency:
  group: phase37121-natural-cross-day-observation-audit
  cancel-in-progress: false

jobs:
  natural-cross-day-observation-audit:
    ...
    steps:
      ...
      - name: Compile Phase 3.7.12.1
        run: python -m py_compile automation/v92/paper_trading_phase37121_natural_cross_day_qualification_observation_evidence_integrity_audit.py

      - name: Validate Phase 3.7.12.1 read-only safety contract
        shell: bash
        run: |
          set -euo pipefail
          f="automation/v92/paper_trading_phase37121_natural_cross_day_qualification_observation_evidence_integrity_audit.py"
          grep -q 'EVIDENCE_SYNTHESIS_ALLOWED = False' "$f"
          grep -q 'CYCLE_DATE_BACKFILL_ALLOWED = False' "$f"
          grep -q 'MANUAL_COUNTER_INCREMENT_ALLOWED = False' "$f"
          grep -q 'QUALIFICATION_THRESHOLD_BYPASS_ALLOWED = False' "$f"
          grep -q 'NATURAL_CROSS_DAY_QUALIFICATION_OBSERVED' "$f"
          grep -q 'NATURAL_CROSS_DAY_OBSERVATION_WAITING' "$f"
          echo "Phase 3.7.12.1 read-only safety contract: PASS"

      - name: Capture observation baseline
        run: python automation/v92/paper_trading_phase37121_natural_cross_day_qualification_observation_evidence_integrity_audit.py --mode baseline

      - name: Wait briefly for stable canonical read
        shell: bash
        run: sleep 5

      - name: Audit current canonical evidence
        id: phase37121
        continue-on-error: true
        run: python automation/v92/paper_trading_phase37121_natural_cross_day_qualification_observation_evidence_integrity_audit.py --mode audit

      - name: Publish summary
        if: always()
        shell: bash
        run: |
          if [ -f artifacts/phase37121/phase37121_summary.md ]; then
            cat artifacts/phase37121/phase37121_summary.md >> "$GITHUB_STEP_SUMMARY"
          fi

      - name: Upload Phase 3.7.12.1 evidence
        if: always()
        uses: actions/upload-artifact@v4
        with:
          name: phase37121-natural-cross-day-observation-audit
          path: artifacts/phase37121
          if-no-files-found: warn
          retention-days: 90

      - name: Enforce Phase 3.7.12.1 audit result
        if: always()
        shell: bash
        run: |
          if [ "${{ steps.phase37121.outcome }}" != "success" ]; then
            echo "Phase 3.7.12.1 natural cross-day evidence audit is BLOCKED."
            exit 1
          fi
          echo "Phase 3.7.12.1 natural cross-day evidence audit is healthy."
'@
...
$required = @(
  ...
  'NATURAL_CROSS_DAY_OBSERVATION_WAITING',
  'NATURAL_CROSS_DAY_QUALIFICATION_OBSERVED',
  'NATURAL_CROSS_DAY_EVIDENCE_AUDIT_BLOCKED',
  'NON_NATURAL_MULTI_COUNTER_JUMP_DETECTED',
  'DUPLICATE_QUALIFICATION_EVIDENCE_DETECTED',
  'CROSS_DAY_EVIDENCE_INTEGRITY_FAILED',
  'EVIDENCE_SYNTHESIS_ALLOWED = False',
  'CYCLE_DATE_BACKFILL_ALLOWED = False',
  'MANUAL_COUNTER_INCREMENT_ALLOWED = False',
  'QUALIFICATION_THRESHOLD_BYPASS_ALLOWED = False',
...
Write-Host "Read-only observation contract: PASS" -ForegroundColor Green
Write-Host "Natural cross-day rollover audit contract: PASS" -ForegroundColor Green
Write-Host "Synthetic evidence prohibition: PASS" -ForegroundColor Green
Write-Host "Cycle-date backfill prohibition: PASS" -ForegroundColor Green
Write-Host "Manual counter increment prohibition: PASS" -ForegroundColor Green
Write-Host "Counter jump guard: PASS" -ForegroundColor Green
Write-Host "Duplicate evidence guard: PASS" -ForegroundColor Green
...
Write-Host ""
Write-Host "PHASE37121 DEPLOYMENT COMPLETE" -ForegroundColor Cyan
...
Write-Host "  $ymlPath"
Write-Host ""
Write-Host "Expected current same-day observation:"
Write-Host "  NATURAL_CROSS_DAY_OBSERVATION_WAITING"
Write-Host "  Observed: 1 -> 1"
Write-Host "  Valid: 1 -> 1"
Write-Host "  Distinct Cycle Dates: 1 -> 1"
Write-Host ""
Write-Host "Expected genuine future cross-day observation:"
Write-Host "  NATURAL_CROSS_DAY_QUALIFICATION_OBSERVED"
...
Write-Host ""
Write-Host "NEXT:"
Write-Host '1. git add "automation/v92/paper_trading_phase37121_natural_cross_day_qualification_observation_evidence_integrity_audit.py"'
Write-Host '2. git add ".github/workflows/gpt-quant-v92-paper-trading-phase37121-natural-cross-day-qualification-observation-evidence-integrity-audit.yml"'
Write-Host '3. git diff --cached --name-only'
Write-Host '4. git commit -m "Deploy Phase 37121 natural cross-day qualification observation evidence integrity audit"'
Write-Host '5. git push origin main'
Write-Host '6. Run a NEW Phase 3.7.12.1 workflow on main.'
