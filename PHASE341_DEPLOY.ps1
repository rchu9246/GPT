...
Write-Host "============================================================"
Write-Host " GPT Quant V9.2 Phase 3.4.1 Deployment"
Write-Host " Daily Qualification Automation"
Write-Host "============================================================"
...
$pyPath = Join-Path $automationDir "paper_trading_phase341_daily_qualification.py"
$ymlPath = Join-Path $workflowDir "gpt-quant-v92-paper-trading-phase341.yml"
...
$workflow = @'
name: GPT Quant V9.2 Paper Trading Phase 3.4.1 - Daily Qualification Automation
  ...
  workflow_dispatch:
    inputs:
      strategy_version:
        ...
        type: string
  schedule:
    # 09:10 UTC = 17:10 Taiwan time. Weekdays only.
    # Qualification reads completed daily snapshots; it does not create a PASS day itself.
    - cron: "10 9 * * 1-5"
...
concurrency:
  group: gpt-quant-v92-phase341-daily-qualification
  cancel-in-progress: false

jobs:
  daily-qualification:
    name: daily-qualification-evaluation-only
    ...
    steps:
      ...
      - name: Validate Phase 3.4.1 safety boundary
        shell: bash
        run: |
          set -euo pipefail
          ...
          grep -q 'SHADOW_ONLY_NO_BROKER' automation/v92/paper_trading_phase341_daily_qualification.py
          grep -q 'automatic_approval.*False' automation/v92/paper_trading_phase341_daily_qualification.py

      - name: Run Phase 3.4.1 Daily Qualification
        run: python automation/v92/paper_trading_phase341_daily_qualification.py

      - name: Upload Phase 3.4.1 diagnostics
        if: always()
        uses: actions/upload-artifact@v4
        with:
          name: phase341-daily-qualification-${{ github.run_id }}
          path: phase341_output/
          if-no-files-found: warn
          retention-days: 14
'@

Set-Content -Path $pyPath -Value $python -Encoding UTF8
Set-Content -Path $ymlPath -Value $workflow -Encoding UTF8
...
Write-Host ""
Write-Host "============================================================"
Write-Host " PHASE 3.4.1 DEPLOYMENT READY"
Write-Host "============================================================"
Write-Host ""
Write-Host "Behavior:"
Write-Host "  Weekdays 17:10 Taiwan time"
Write-Host "  Runs Phase 3.4 evaluate only"
Write-Host "  PASS-day source remains distinct_run_date_snapshot_status"
Write-Host "  Re-running the same day does NOT create another PASS day"
Write-Host "  5/5 may qualify, but NEVER auto-approves release"
Write-Host ""
Write-Host "Safety:"
...
Write-Host "  2. Confirm the two new Phase 3.4.1 files"
Write-Host "  3. Commit to main"
Write-Host "  4. Push origin"
Write-Host "  5. GitHub Actions -> Phase 3.4.1 -> Run workflow once manually"
