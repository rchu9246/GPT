...
Write-Host "PHASE375 V1 - Runtime Supervision Canonical Compatibility Fix" -ForegroundColor Cyan
...
$backup = Join-Path $repo ".phase375-v1-runtime-compat-backup-$stamp"
New-Item -ItemType Directory -Force -Path $backup | Out-Null
Copy-Item $pyPath (Join-Path $backup (Split-Path $pyPath -Leaf))
Copy-Item $ymlPath (Join-Path $backup (Split-Path $ymlPath -Leaf))
...
[System.IO.File]::WriteAllText($pyPath, $pyText + [Environment]::NewLine, (New-Object System.Text.UTF8Encoding($false)))
[System.IO.File]::WriteAllText($ymlPath, $ymlText + [Environment]::NewLine, (New-Object System.Text.UTF8Encoding($false)))

python -m py_compile $pyPath
if ($LASTEXITCODE -ne 0) { throw "Python compile failed." }

$verify = Get-Content -Raw $pyPath
$required = @(
  'RUNTIME_SUPERVISION_READY',
  'RUNTIME_CANONICAL_READY',
  'PRODUCTION_PAPER_RUNTIME_ACTIVE',
...
foreach ($token in $required) {
  if ($verify -notmatch [regex]::Escape($token)) { throw "Verification failed: $token" }
}

Write-Host ""
Write-Host "PHASE375 V1 RUNTIME SUPERVISION CANONICAL COMPATIBILITY FIX DEPLOYMENT COMPLETE" -ForegroundColor Cyan
...
Write-Host '4. git commit -m "Fix Phase 375 runtime supervision canonical compatibility"'
Write-Host '5. git push origin main'
Write-Host '6. Re-run Phase 3.7.5 GitHub Action.'
