...
Write-Host "============================================================"
Write-Host " GPT Quant Phase 3.4.4"
Write-Host " Human Approval Readiness Gate"
Write-Host "============================================================"
...
$pyPath = Join-Path $automationDir "paper_trading_phase344_human_approval_readiness_gate.py"
$ymlPath = Join-Path $workflowDir "gpt-quant-v92-paper-trading-phase344-human-approval-readiness-gate.yml"
...
jobs:
  human-approval-readiness:
    ...
    steps:
      ...
      - name: Validate Phase 3.4.4 safety boundary
        shell: bash
        run: |
          set -euo pipefail
          test -n "${SUPABASE_URL:-}"
          ...
          grep -q SHADOW_ONLY_NO_BROKER automation/v92/paper_trading_phase344_human_approval_readiness_gate.py
          grep -q '"release_authorized": False' automation/v92/paper_trading_phase344_human_approval_readiness_gate.py
          grep -q '"automatic_approval": False' automation/v92/paper_trading_phase344_human_approval_readiness_gate.py
...
Write-Host "  < 5 PASS days  => NOT_READY / CLOSED_WAITING_FOR_QUALIFICATION"
Write-Host "  >=5 PASS days  => READY_FOR_HUMAN_APPROVAL / OPEN_FOR_HUMAN_REVIEW"
Write-Host "  Release remains LOCKED in all cases"
Write-Host ""
Write-Host "Safety:"
