...
Write-Host ""
Write-Host "========================================================="
Write-Host " GPT Quant V9.2 Phase 3.2 Deployment"
Write-Host " Production Validation + Promotion Gate"
Write-Host "========================================================="
Write-Host ""

$RepoRoot = (Get-Location).Path
$AutomationDir = Join-Path $RepoRoot "automation\v92"
$WorkflowDir = Join-Path $RepoRoot ".github\workflows"

New-Item -ItemType Directory -Force -Path $AutomationDir | Out-Null
New-Item -ItemType Directory -Force -Path $WorkflowDir | Out-Null

$PythonPath = Join-Path $AutomationDir "paper_trading_phase32_validation_promotion_gate.py"
$WorkflowPath = Join-Path $WorkflowDir "gpt-quant-v92-paper-trading-phase32.yml"
    ...
    steps:
      ...
      - name: Validate Phase 3.2 environment
        ...
        run: |
          ...
          test -f automation/v92/paper_trading_phase32_validation_promotion_gate.py || {
            echo "::error::Missing Phase 3.2 Python file"
            exit 1
          }
          if [ "${PAPER_TRADING_MODE}" != "SHADOW_ONLY_NO_BROKER" ]; then
            echo "::error::Safety lock failed"
            exit 1
          fi

      - name: Run Phase 3.2 Production Validation + Promotion Gate
        shell: bash
        run: |
          set -o pipefail
          mkdir -p phase32_logs
          python automation/v92/paper_trading_phase32_validation_promotion_gate.py 2>&1 | tee phase32_logs/phase32.log

      - name: Upload Phase 3.2 diagnostics
        if: always()
        uses: actions/upload-artifact@v4
        with:
          name: phase32-production-validation-${{ github.run_id }}
          path: phase32_logs/
          if-no-files-found: warn
          retention-days: 90
'@

Set-Content -Path $PythonPath -Value $PythonContent -Encoding UTF8
Set-Content -Path $WorkflowPath -Value $WorkflowContent -Encoding UTF8

if (-not (Test-Path $PythonPath)) {
    throw "Phase 3.2 Python file was not created."
}

if (-not (Test-Path $WorkflowPath)) {
    throw "Phase 3.2 workflow file was not created."
}
...
Write-Host ""
Write-Host "========================================================="
Write-Host " PHASE 3.2 DEPLOYMENT READY"
Write-Host "========================================================="
Write-Host ""
Write-Host "Next:"
Write-Host "  1. Open GitHub Desktop"
Write-Host "  2. Confirm the two new Phase 3.2 files"
Write-Host "  3. Commit to main"
Write-Host "  4. Push origin"
Write-Host "  5. GitHub Actions -> Phase 3.2 -> Run workflow -> V9.1"
Write-Host ""
