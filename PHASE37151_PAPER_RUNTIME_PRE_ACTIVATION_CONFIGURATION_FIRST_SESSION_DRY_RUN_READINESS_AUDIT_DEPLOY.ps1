...
Write-Host "PHASE37151 - Paper Runtime Pre-Activation Configuration + First Session Dry-Run Readiness Audit" -ForegroundColor Cyan
Write-Host "Mode: READ-ONLY / SIDE-EFFECT-FREE PAPER DRY-RUN" -ForegroundColor Green
Write-Host "Safety boundary: PAPER ONLY / broker submission DISABLED / real money DISABLED" -ForegroundColor Green

$repo = (Get-Location).Path
$pyRel = "automation/v92/paper_trading_phase37151_paper_runtime_pre_activation_configuration_first_session_dry_run_readiness_audit.py"
$ymlRel = ".github/workflows/gpt-quant-v92-paper-trading-phase37151-paper-runtime-pre-activation-configuration-first-session-dry-run-readiness-audit.yml"

$pyPath = Join-Path $repo $pyRel
$ymlPath = Join-Path $repo $ymlRel

$stamp = Get-Date -Format "yyyyMMdd-HHmmss"
$backup = Join-Path $repo ".phase37151-backup-$stamp"
...
'@
$ymlText = @'
name: GPT Quant Phase 3.7.15.1 - Paper Runtime Pre-Activation Configuration First Session Dry-Run Readiness Audit

on:
  workflow_dispatch:
  schedule:
    # Weekdays 18:05 UTC / 02:05 Asia-Taipei.
    - cron: "5 18 * * 1-5"
...
concurrency:
  group: phase37151-paper-runtime-pre-activation-dry-run-audit
  cancel-in-progress: false

jobs:
  paper-runtime-pre-activation-dry-run-audit:
    ...
    env:
      ...
      WF3715: gpt-quant-v92-paper-trading-phase3715-production-paper-runtime-activation-first-live-paper-session-safety-validation.yml
    ...
    steps:
      ...
      - name: Compile Phase 3.7.15.1
        run: python -m py_compile automation/v92/paper_trading_phase37151_paper_runtime_pre_activation_configuration_first_session_dry_run_readiness_audit.py

      - name: Validate Phase 3.7.15.1 dry-run safety contract
        shell: bash
        run: |
          set -euo pipefail
          f="automation/v92/paper_trading_phase37151_paper_runtime_pre_activation_configuration_first_session_dry_run_readiness_audit.py"
          ...
          grep -q 'DRY_RUN_BROKER_SIDE_EFFECT_ALLOWED = False' "$f"
          grep -q 'DRY_RUN_REAL_MONEY_SIDE_EFFECT_ALLOWED = False' "$f"
          grep -q 'PRE_ACTIVATION_BYPASS_ALLOWED = False' "$f"
          grep -q 'PAPER_RUNTIME_PRE_ACTIVATION_AUDIT_READY_WAITING_FOR_3OF3' "$f"
          grep -q 'PAPER_RUNTIME_PRE_ACTIVATION_AUDIT_READY_FOR_FIRST_SESSION' "$f"
          echo "Phase 3.7.15.1 dry-run safety contract: PASS"

      - name: Confirm Phase 3.7.15 successful lineage
        shell: bash
        env:
        ...
        run: |
          ...
          result="$(gh api \
            ...
            "/repos/${GITHUB_REPOSITORY}/actions/workflows/${WF3715}/runs?branch=main&per_page=20" \
            --jq '[.workflow_runs[] | select(.conclusion == "success")][0].conclusion // empty')"

          if [ "$result" != "success" ]; then
            echo "::error::No successful Phase 3.7.15 run found on main."
            exit 1
          fi

          echo "Phase 3.7.15 successful lineage: PASS"

      - name: Execute pre-activation configuration and dry-run readiness audit
        id: phase37151
        continue-on-error: true
        run: python automation/v92/paper_trading_phase37151_paper_runtime_pre_activation_configuration_first_session_dry_run_readiness_audit.py

      - name: Publish summary
        if: always()
        shell: bash
        run: |
          if [ -f artifacts/phase37151/phase37151_summary.md ]; then
            cat artifacts/phase37151/phase37151_summary.md >> "$GITHUB_STEP_SUMMARY"
          fi

      - name: Upload Phase 3.7.15.1 evidence
        if: always()
        uses: actions/upload-artifact@v4
        with:
          name: phase37151-paper-runtime-pre-activation-dry-run-audit
          path: artifacts/phase37151
          if-no-files-found: warn
          retention-days: 90

      - name: Enforce Phase 3.7.15.1 result
        if: always()
        shell: bash
        run: |
          if [ "${{ steps.phase37151.outcome }}" != "success" ]; then
            echo "Phase 3.7.15.1 pre-activation configuration/dry-run audit is BLOCKED."
            exit 1
          fi
          echo "Phase 3.7.15.1 pre-activation configuration/dry-run audit is healthy."
'@
...
$required = @(
  ...
  'PAPER_RUNTIME_PRE_ACTIVATION_AUDIT_READY_WAITING_FOR_3OF3',
  'PAPER_RUNTIME_PRE_ACTIVATION_AUDIT_READY_FOR_FIRST_SESSION',
  'PAPER_RUNTIME_PRE_ACTIVATION_AUDIT_BLOCKED',
  'QUALIFICATION_MUTATION_ALLOWED = False',
  'SYNTHETIC_CYCLE_DATE_ALLOWED = False',
  'MANUAL_COUNTER_INCREMENT_ALLOWED = False',
  'DRY_RUN_BROKER_SIDE_EFFECT_ALLOWED = False',
  'DRY_RUN_REAL_MONEY_SIDE_EFFECT_ALLOWED = False',
  'PRE_ACTIVATION_BYPASS_ALLOWED = False',
  'gpt-quant-v92-paper-trading-phase3715-production-paper-runtime-activation-first-live-paper-session-safety-validation.yml'
)
...
Write-Host "Phase 3.7.15 lineage bridge: PASS" -ForegroundColor Green
Write-Host "Pre-activation configuration audit contract: PASS" -ForegroundColor Green
Write-Host "First session dry-run contract: PASS" -ForegroundColor Green
...
Write-Host "Dry-run broker side-effect prohibition: PASS" -ForegroundColor Green
Write-Host "Dry-run real-money side-effect prohibition: PASS" -ForegroundColor Green
Write-Host "Broker/real-money safety lock: PASS" -ForegroundColor Green
Write-Host ""
Write-Host "PHASE37151 DEPLOYMENT COMPLETE" -ForegroundColor Cyan
...
Write-Host "  PAPER_RUNTIME_PRE_ACTIVATION_AUDIT_READY_WAITING_FOR_3OF3"
Write-Host "  Configuration Audit: PASS"
Write-Host "  Dry Run: PASS"
Write-Host "  Qualification remains natural; no mutation is performed"
Write-Host ""
Write-Host "After genuine canonical 3/3 + Phase 3.7.15 readiness:"
Write-Host "  PAPER_RUNTIME_PRE_ACTIVATION_AUDIT_READY_FOR_FIRST_SESSION"
Write-Host ""
...
Write-Host "  Weekdays 18:05 UTC / 02:05 Asia-Taipei"
Write-Host ""
Write-Host "Backup: $backup"
Write-Host ""
Write-Host "NEXT:"
Write-Host '1. git add "automation/v92/paper_trading_phase37151_paper_runtime_pre_activation_configuration_first_session_dry_run_readiness_audit.py"'
Write-Host '2. git add ".github/workflows/gpt-quant-v92-paper-trading-phase37151-paper-runtime-pre-activation-configuration-first-session-dry-run-readiness-audit.yml"'
Write-Host '3. git diff --cached --name-only'
Write-Host '4. git commit -m "Deploy Phase 37151 paper runtime pre-activation configuration first session dry-run readiness audit"'
Write-Host '5. git push origin main'
Write-Host '6. Run a NEW Phase 3.7.15.1 workflow on main.'
